...
public class oreCarMove1 : MonoBehaviour
{
    ...
    void FindVisableOrefield()
    {
        ...
        if (this.visableOrefields.Count > 0 && visableOrefields[0])
        {
            ...
            //抵达最近的矿源
            if (oreCar != null && visableOrefields[minIndex] != null && oreSetting.oreMaxPower > 0)
            {
            ...
            {
                if (visableOrefields[minIndex] == createOrefields.cloneOrefields[i].transform)
                {
    ...
    private GameObject Ori_Orefield;

    [Header("次数控制")]
    public float activeTime;//显示的时间(采矿达到5次消失)
    public float activeStart;//开始显示时间

    //[Header()]
    private void onEnable()
    {
        Ori_Orefield_transform = GameObject.FindGameObjectWithTag("Orefield").transform;
...
{
    public static OrefieldPool instance;

    public GameObject prefab_Orefield;

    public int maxOrefieldCount;
    private Queue<GameObject> availableOrefields = new Queue<GameObject>();//池
    ...
    public void FillPool()
    {
        ...
        {
            var _Orefield = Instantiate(prefab_Orefield);
            _Orefield.transform.SetParent(transform);
        }
    }
}
